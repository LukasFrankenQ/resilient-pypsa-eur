"""v2 of the total system cost vs. gas consumption figure.

Difference from plot_cost_price_surfaces.py: only the default (base) gas-price
sweep is read from actual network files; it is extended out to 4500 TWh. The
four higher-gas-price curves are NOT taken from `gas+Generator+m*` model files
(those lv1.25_3H variants do not exist). Instead each base data point is
re-priced: holding the same physical model (same gas use and ETS payments), the
extra resource cost of running gas at price g instead of the base price is
    (g - base_price)[€/MWh] * gas_consumption[TWh] / 1000  ->  B€/a.
The four target prices (30.7, 36.9, 43.0, 49.1 €/MWh) are the gas prices that
the m1.25/1.5/1.75/2.0 wildcard tunings produced in the original figure. The
plot layout is otherwise identical.

Only wiggle values from the base sweep are used, capped at
4500 TWh. For each sweep a smooth curve is fitted whose slope is
...
# --- only the base sweep uses real networks; higher-gas-price sweeps are
#     synthesised by re-pricing each base point's gas use ---
base_sweep = next(sw for sw in SWEEPS if sw["key"] == "base")
common = sorted(w for w in discover_wiggles(base_sweep["pattern"]) if w % 500 == 0)
print(f"base wiggles (<= {WIGGLE_MAX}, 500-steps): {common}")
if not common:
    raise SystemExit("No base (default gas price) networks found.")

# --- load base networks (cached on (mtime, size)) ---
cache = load_cache()
...
for w in common:
    for p in NET_DIR.iterdir():
        m = base_sweep["pattern"].match(p.name)
        if m and int(m.group(1)) == w:
            rec = get_metrics(p, cache)
            wiggles.append(w)
            costs.append(rec["cost_BEUR"])
            ets_costs.append(rec["co2_ets_cost_BEUR"])
            if base_price is None and rec["gas_marginal_cost"] is not None:
                base_price = rec["gas_marginal_cost"]
            break
save_cache(cache)
...
print(f"  base: gas price = {base_price:.1f} EUR/MWh, n = {len(wiggles)}")

# Target gas prices that the m1.25/1.5/1.75/2.0 wildcard tunings produced in the
# original figure. The base point at gas consumption x carries gas cost at
# base_price; repricing to g (same physical model) adds
#   (g - base_price)[€/MWh] * x[TWh] / 1000  ->  B€/a.
SYNTH_PRICES = {"m1.25": 30.7, "m1.5": 36.9, "m1.75": 43.0, "m2.0": 49.1}

results = {}
results["base"] = dict(
    wiggles=wiggles, costs=costs, ets_costs=ets_costs,
    gas_price=base_price, color=base_sweep["color"],
)
for sw in SWEEPS:
    if sw["key"] == "base":
        continue
    g = SYNTH_PRICES[sw["key"]]
    synth_costs = costs + (g - base_price) * wiggles / 1000.0
    results[sw["key"]] = dict(
        wiggles=wiggles.copy(), costs=synth_costs, ets_costs=ets_costs.copy(),
        gas_price=g, color=sw["color"],
    )
    print(f"  {sw['key']}: synthesised gas price = {g:.1f} EUR/MWh "
          f"(+{g - base_price:.1f} vs base)")
...
def plot_panel(ax, results_dict, minima_label, y_lo, y_hi, label_offset=(0, 0),
    ...
    for sw in SWEEPS:
        ...
        # m1.25 (30.7 €/MWh) line is pale yellow -> use black for its label
        _label_color = "black" if sw["key"] == "m1.25" else r["color"]
        ax.text(x_end, y_end + 0.5, f"{r['gas_price']:.1f}\n€/MWh",
                color=_label_color, va="bottom", ha="right",
...
# "Today's Demand" grey band (3500-4500), matching main_results.pdf
ax_top.axvspan(3500, 4500, color="grey", alpha=0.4, zorder=-1)
ax_top.text(4000, y_hi_t - 0.03 * y_range_t - 10, "Today's\nDemand",
            color="dimgray", alpha=0.95, fontsize=9,
            ha="center", va="top", linespacing=0.95, zorder=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="white",
                      edgecolor="0.4", linewidth=0.8, alpha=1.0))
...
fig.tight_layout()
out = IMG_DIR / "cost_vs_gas_consumption_v2.pdf"
...
# "Today's Demand" grey band (3500-4500), matching main_results.pdf
ax2.axvspan(3500, 4500, color="grey", alpha=0.4, zorder=-1)
ax2.text(4000, y_hi_t - 0.03 * y_range_t - 10, "Today's\nDemand",
         color="dimgray", alpha=0.95, fontsize=9,
         ha="center", va="top", linespacing=0.95, zorder=10,
         bbox=dict(boxstyle="round,pad=0.3", facecolor="white",
                   edgecolor="0.4", linewidth=0.8, alpha=1.0))
...
fig2.tight_layout()
out2 = IMG_DIR / "cost_vs_gas_consumption_annotated_v2.pdf"
...
export_frontend_data("cost_vs_gas_consumption_v2", {
